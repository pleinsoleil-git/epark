	...
	static class _Current {
		...
		Collection<Object[]> query() throws Exception {
			...
			sql = "SELECT t10.data_type,\n"
				...
				+ "CASE\n"
					+ "WHEN t10.usage_within_last_month = 0 THEN 0\n"
					+ "ELSE TRUNC( t10.usage_within_after_month / t10.usage_within_last_month, 3 )\n"
				+ "END\n"
			+ "FROM\n"
			+ "(\n"
				+ "SELECT NULL::VARCHAR AS data_type,\n"
					...
					+ "t10.all_usage_within_last_6_month\n"
						...
						+ "+ COALESCE( LAG( t10.all_usage_within_last_6_month, 2 ) OVER ( ORDER BY t10.usage_month ), 0 ) AS usage_within_last_month,\n"
					+ "t10.usage_within_after_60_day\n"
						+ "+ COALESCE( LAG( t10.usage_within_after_60_day, 1 ) OVER ( ORDER BY t10.usage_month ), 0 )\n"
						+ "+ COALESCE( LAG( t10.usage_within_after_60_day, 2 ) OVER ( ORDER BY t10.usage_month ), 0 ) AS usage_within_after_month\n"
				...
				+ "SELECT 'ALL',\n"
					...
					+ "t10.all_usage_within_last_6_month\n"
						+ "+ COALESCE( LAG( t10.all_usage_within_last_6_month, 1 ) OVER ( ORDER BY t10.usage_month ), 0 )\n"
						+ "+ COALESCE( LAG( t10.all_usage_within_last_6_month, 2 ) OVER ( ORDER BY t10.usage_month ), 0 ),\n"
					+ "t10.all_usage_within_after_60_day\n"
						+ "+ COALESCE( LAG( t10.all_usage_within_after_60_day, 1 ) OVER ( ORDER BY t10.usage_month ), 0 )\n"
						+ "+ COALESCE( LAG( t10.all_usage_within_after_60_day, 2 ) OVER ( ORDER BY t10.usage_month ), 0 )\n"
				+ "FROM tmp_repeat_report AS t10\n"
			+ ") AS t10\n"
